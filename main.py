...
def plot_3D_gradient_descent(f_x, f_y, descent_path, domain) -> None:
    ...
    for i in range(Z.shape[0]):
        for j in range(Z.shape[1]):
            Z[i, j] = poly.get_multivariable_polynomial_value(
                f_x, f_y, X[i, j], Y[i, j])
...
def create_descent_path(
    ...
    start_value = poly.get_multivariable_polynomial_value(
    ...
    for i in range(iterations):
        ...
        descent_path.append((current_x, current_y, poly.get_multivariable_polynomial_value(
